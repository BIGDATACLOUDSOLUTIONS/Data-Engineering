...
Answer: We can use row_number and MOD function to retrieve the alternate records from a table.
To get Even number records:
SELECT * 
FROM (SELECT ID, Salary, row_number() over(order by ID) as position 
FROM Employee) 
...
If Yes:
select d.DeptName,nvl(max(e.salary),0) as max_sal 
from department d left outer join employee e on e.DeptId=d.id group by d.DeptName
If No:
select d.DeptName,nvl(max(e.salary),0) as max_sal 
from department d inner join employee e on e.DeptId=d.id group by d.DeptName
```

```text
Q. Write a SQL query to find records in Table A that are not in Table B.
Input Table: 
Table A:
+---+
| id|
...
Answer:
In SQL null can not be compared with itself. There fore null = null is not true. We can compare null with a non-null value to check whether a value is not null.
...
Answer: We can first use group by DeptID on Employee table and then get the Max salary from each Dept group.
...
```text
Q. How can you find 10 employees with Even number as Employee ID?
Answer:
In Hive we can use LIMIT to limit the number of records. We can also use row_number < 11 to get the only 10 or less number of records.
To find the Even number Employee ID, we can use % function.
Answer:
SELECT ID FROM Employee WHERE ID % 2 = 0 LIMIT 10
```

```text
Q. Write a SQL Query to get the names of employees whose date of birth is between 01/01/1990 to 31/12/2000.
Answer: This SQL query appears a bit tricky. We can use BETWEEN clause to get all the employees whose date of birth lies between two given dates.
Query will be as follows:
SELECT EmpName
FROM Employees
...
Answer: Default Data Format in Hive: yyyy-MM-dd HH:mm:ss
select quarter(concat_ws('-',date_part[2],date_part[0],date_part[1])) as Q from (SELECT split('03/31/2016','/') as date_part)
```
...
Query would be as follows:
SELECT collect_list(name) as name_lst, COUNT(email)
FROM Employee
GROUP BY email
...
Answer: A Pseudocolumn is like a table column, but it is not stored in the same table. We can select from a Pseudocolumn, but we can not insert, update or delete on a Pseudocolumn.
A Pseudocolumn is like a function with no arguments.
Two most popular Pseudocolumns in Oracle are row_number and rank.
```
...
In SQL, we can write if else as well CASE when clauses.
```
Section 5: 
```text
What is the difference between DELETE and TRUNCATE in SQL?
Main differences between DELETE and TRUNCATE commands are:
DML vs. DDL: DELETE is a Data Manipulation Language (DML) command.  
...
Answer: Note that, starting from Hive 2.2, merge statement is supported in Hive if you create transaction table.
...
SELECT *
FROM Employee
WHERE id NOT IN (SELECT Emp_id FROM Department)
...
SELECT *
FROM Employee
WHERE id NOT IN (SELECT Emp_id FROM Department WHERE Emp_id IS NOT NULL)
```
...
SELECT Name ROM Employee WERE DeptID <> 1;
Employee Table:
Id         Name               DeptID
1          John               NULL
2          George             2
...
```text
What are the differences between CASE and DECODE in SQL?Main differences between Case and Decode statements are:
