...
object NewSupplierConsumer {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var consumer: KafkaConsumer[String, Supplier] = null
    ...
    try {
      ...
      while (true) {
        ...
        for (record <- records) {
          println("Supplier id= " + String.valueOf(record.value().getID) + " Supplier  Name = " + record.value().getName + " Supplier Start Date = " + record.value().getStartDate.toString)
        }
      }
    }
  }
